...
typedef struct AVFilter {
     ...
     *
     * This callback will be called immediately after the filter m_context is
...
struct AVFilterContext {
     ...
     * hardware m_context information.
     */
 ...
 *
 * @param ctx  uninitialized filter m_context to initialize
...
/**
 * Free a filter m_context. This will also remove the filter from its
 ...
 *
 * @param ctx the filter m_context the job belongs to
 ...
 *
 * @param ctx the filter m_context to which the jobs belong
 ...
 *
 * @return the m_context of the newly created filter instance (note that it is
 ...
 * @param log_ctx m_context used for logging
 ...
 * filter m_context and the pad index required for establishing a link.
 */
typedef struct AVFilterInOut {
    /** unique name for this input/output in the list */
    char *name;

    /** filter m_context associated to this input/output */
 ...
 *
 * @param graph   the filter graph where to link the parsed graph m_context
 ...
 * Add a graph described by a string to a graph.
 *
 * @param[in]  graph   the filter graph where to link the parsed graph m_context
 ...
 * AVFilterParams.filter is set to the newly-created filter m_context,
 ...
 * no associated filter m_context are simply skipped.
 *
 * @param seg the filtergraph segment to process
 * @param flags reserved for future use, caller must set to 0 for now
 *
 ...
 * Any creation-pending filters (see avfilter_graph_segment_create_filters())
 * present in the segment will cause this function to fail. AVFilterParams with
 * no associated filter m_context or whose filter m_context is already initialized,
 * are simply skipped.
 *
 * @param seg the filtergraph segment to process
 * @param flags reserved for future use, caller must set to 0 for now
