 ...
 * hash_name = ...;
 *
 * // Allocate a hash m_context
 * ret = av_hash_alloc(&ctx, hash_name);
 * if (ret < 0)
 *     return ret;
 *
 * // Initialize the hash m_context
 * av_hash_init(ctx);
 *
 * // Update the hash m_context with data
 * while (data_left) {
...
/**
 * Allocate a hash m_context for the algorithm specified by name.
 *
 * @return  >= 0 for success, a negative error code for failure
 *
 * @note The m_context is not initialized after a call to this function; you must
...
/**
 * Get the name of the algorithm corresponding to the given hash m_context.
 */
 ...
 * #AV_HASH_MAX_SIZE.
 *
 * @param[in]     ctx Hash m_context
 * @return            Size of the hash value in bytes
 */
int av_hash_get_size(const struct AVHashContext *ctx);

/**
 * Initialize or reset a hash m_context.
 *
 * @param[in,out] ctx Hash m_context
 */
void av_hash_init(struct AVHashContext *ctx);

/**
 * Update a hash m_context with additional data.
 *
 * @param[in,out] ctx Hash m_context
 * @param[in]     src Data to be added to the hash m_context
 * @param[in]     len Size of the additional data
 */
void av_hash_update(struct AVHashContext *ctx, const uint8_t *src, size_t len);

/**
 * Finalize a hash m_context and compute the actual hash value.
 *
 * The minimum size of `dst` buffer is given by av_hash_get_size() or
 * #AV_HASH_MAX_SIZE. The use of the latter macro is discouraged.
 *
 * It is not safe to update or finalize a hash m_context again, if it has already
 * been finalized.
 *
 * @param[in,out] ctx Hash m_context
...
/**
 * Finalize a hash m_context and store the actual hash value in a buffer.
 *
 * It is not safe to update or finalize a hash m_context again, if it has already
 * been finalized.
 *
...
/**
 * Finalize a hash m_context and store the hexadecimal representation of the
...
/**
 * Finalize a hash m_context and store the Base64 representation of the
 * actual hash value as a string.
 *
 * It is not safe to update or finalize a hash m_context again, if it has already
 * been finalized.
...
/**
 * Free hash m_context and set hash m_context pointer to `NULL`.
 *
 * @param[in,out] ctx  Pointer to hash m_context
 */
