...
#ifndef FFMPEGNDKVIDEO_IMAGEDEF_H
#define FFMPEGNDKVIDEO_IMAGEDEF_H
...
public:
    ...
    static NativeImage* cloneNativeImage(NativeImage *pSrcImg) {
...
//        NativeImageUtil::FreeNativeImage(&renderImage);
  ...
  //      memset(&renderImage, 0, sizeof(NativeImage));
        renderImage->format = pSrcImg->format;
        renderImage->width = pSrcImg->width;
        renderImage->height = pSrcImg->height;
        renderImage->pts = pSrcImg->pts;
        renderImage->rotation = pSrcImg->rotation;
        NativeImageUtil::AllocNativeImage(renderImage);
        NativeImageUtil::CopyNativeImage(pSrcImg, renderImage);
    ...
    static void CopyNativeImage(NativeImage *pSrcImg, NativeImage *pDstImg)
    {
        ...
        if(pSrcImg->format != pDstImg->format ||
           ...
           pSrcImg->rotation != pDstImg->rotation)
        {
...
#endif //FFMPEGNDKVIDEO_IMAGEDEF_H
