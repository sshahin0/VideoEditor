...
{
  DataType fCMax = std::fmax(std::fmax(red(), green()), blue());
  DataType fCMin = std::fmin(std::fmin(red(), green()), blue());
...
{
  DataType fCMin = std::fmin(red(), std::fmin(green(), blue()));
  DataType fCMax = std::fmax(red(), std::fmax(green(), blue()));
...
{
  unsigned int count = std::fmin(RGBA, nb_channels);
