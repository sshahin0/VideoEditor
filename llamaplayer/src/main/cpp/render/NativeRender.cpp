...
void NativeRender::Init(int videoWidth, int videoHeight, int *dstSize)
{
    ...
    LOGCATE("NativeRender::Init window[w,h]=[%d, %d],DstSize[w, h]=[%d, %d] %d", windowWidth, windowHeight, m_DstWidth, m_DstHeight,
            this);
...
void NativeRender::RenderVideoFrame(NativeImage *pImage)
{
  ...
  //  LOGCATE("before loop %lld, %d", m_DstHeight, this);
