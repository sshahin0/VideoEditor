...
VideoGLRender::~VideoGLRender() {
    LOGCATD("native video gl render destructor");
...
/*glGenTextures(TEXTURE_NUM, m_TextureIds);
    ...
    for(int i = 0; i < TEXTURE_NUM; i++) {
     //   textures[i] = Vivid::Texture(Vivid::TextureType::_2D);
        textures[i].setup();
    }
...
void VideoGLRender::OnDrawFrame() {
    ...
    LOGCATD("native on draw frame program %d, plane %p", m_ProgramObj, m_RenderImage.ppPlane[0]);
...
//        NativeImageUtil::DumpNativeImage(&m_RenderImage, "/sdcard", "2222");
    ...
    // upload image data
    {
        ...
        textureParams.format = m_RenderImage.format;
...
/*glActiveTexture(GL_TEXTURE0);
                ...
                textureParams.width = m_RenderImage.width;
                textureParams.height = m_RenderImage.height;
                textures[0].bind(0);
                textures[0].loadFrame(textureParams, m_RenderImage.ppPlane[0]);
                textures[0].unbind();
...
/*glActiveTexture(GL_TEXTURE1);
                ...
                textureParams.width = m_RenderImage.width >> 1;
                textureParams.height = m_RenderImage.height >> 1;
                textures[1].bind(1);
                textures[1].loadFrame(textureParams, m_RenderImage.ppPlane[1]);
                textures[1].unbind();
...
/*glActiveTexture(GL_TEXTURE2);
                ...
                textureParams.height = m_RenderImage.height >> 1;
                textures[2].bind(2);
                textures[2].loadFrame(textureParams, m_RenderImage.ppPlane[2]);
                textures[2].unbind();
...
/*glActiveTexture(GL_TEXTURE0 + i);
        ...
        LOGCATD("for loop %d, %d", i, textures[i].getTexName());
...
void VideoGLRender::ReleaseInstance() {
    if(s_Instance != nullptr)
    {
        ...
        if(s_Instance != nullptr)
        {
            LOGCATD("native gl render release");
            delete s_Instance;
            s_Instance = nullptr;
        }
